...
module MavisCLI
  module Reports
    class SendToCareplus < Dry::CLI::Command
      ...
      DEFAULT_ENDPOINT = "http://localhost:8080/MOCK/soap.SchImms.cls"

      # TODO: replace with real credentials before production use
      ...
      def call(
        ...
        **
        ...
        username, password = resolve_credentials(ods_code:, workgroup:)
        return if username.nil?

        csv_payload = File.read(input)

        soap_body = build_soap_envelope(csv_payload, username:, password:)
      ...
      def resolve_credentials(ods_code:, workgroup:)
        return FALLBACK_USERNAME, FALLBACK_PASSWORD if ods_code.nil?
        ...
        [team.careplus_username, team.careplus_password]
      end

      def build_soap_envelope(csv_payload, username:, password:)
