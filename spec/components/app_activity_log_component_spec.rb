...
describe AppActivityLogComponent do
  [true, false].each do |flag_enabled|
    context "with one_patient_per_parent feature flag #{flag_enabled ? "enabled" : "disabled"}" do
      ...
      let(:user) do
        create(:user, team:, family_name: "Joy", given_name: "Nurse")
      end
      let(:location) do
      ...
      shared_examples "card" do |title:, date:, notes: nil, by: nil, index: nil, programme: nil|
        it "renders card '#{title}'" do
          ...
          if by
            expect(card).to have_css("p.app-timeline__description", text: by)
          end
      ...
      describe "gillick assessments" do
        ...
        include_examples "card",
                         title:
                           "Completed Gillick assessment as Gillick competent",
                         notes: "First notes",
        ...
        context "for a not administered record" do
          before do
            create(
              ...
              notes: "Some notes.",
              vaccine:
                programmes.first.vaccines.find_by!(upload_name: "Gardasil")
            )
          end
